          ...
          <div className="relative w-[290px] md:w-[450px] aspect-[4/3]">
            {images.map((img, idx) => (
              <img
                ...
                className={`absolute top-0 left-0 w-full h-full object-cover rounded-2xl shadow-2xl transition-opacity duration-1000 ${
          ...
          <div className="flex flex-col gap-5">
            {topics.map((topic, idx) => (
              <div key={idx} className="flex items-start gap-3">
                <p className="text-[clamp(0.9rem,1.6vw,1rem)] font-semibold text-gray-700 text-justify leading-relaxed">
                  {topic}
                </p>
              </div>
            ))}
